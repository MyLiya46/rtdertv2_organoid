...
class RTDETRTransformerv2(nn.Module):
    ...
    # def _feature_merge(self, feats: List[torch.Tensor], skip: List[torch.Tensor]):
    #     feats_merge = []
    #     device = feats[0].device
    #     for idx in range(len(feats)):
    #         c_feats = feats[idx].shape[1]
    #         c_skip = skip[idx].shape[1]
    #         h = w = feats[idx].shape[2]
    #         dwcon = nn.Sequential(nn.Conv2d(c_skip, c_skip, kernel_size=3, padding=1, groups=c_skip, bias=False).to(device),
    #                               nn.BatchNorm2d(c_skip).to(device),
    #                               nn.ReLU(inplace=True).to(device),
    #                               nn.Conv2d(c_skip, c_feats, kernel_size=1, bias=False).to(device),
    #                               nn.BatchNorm2d(c_feats).to(device),
    #                               nn.ReLU(inplace=True).to(device))
    #         skip[idx] = dwcon(skip[idx])
    #         feat_con = torch.concat([feats[idx], skip[idx]], dim=1)
    #         pointwise_1 = nn.Sequential(nn.Conv2d(feat_con.shape[1], c_feats, kernel_size=1, bias=False).to(device),
    #                                     nn.BatchNorm2d(c_feats).to(device),
    #                                     nn.ReLU(inplace=True).to(device))
    #
    #         feat_con = pointwise_1(feat_con)
    #         feats_merge.append(feat_con)
    #     return feats_merge

    def forward(self, feats, targets=None):
        # input projection and embedding
        # feats_merge = self._feature_merge(feats, skip)
